...
def _content_summary(item: ContentItem) -> ContentAdminSummary:
...
def _get_content(db: Session, content_id: str) -> ContentItem:
    statement = (
        select(ContentItem)
...
def create_user(
    ...
    try:
        ...
        encoded = hash_password(payload.password, minimum_length=settings.password_min_length)
...
def update_user(
    ...
    owner_count = db.scalar(select(func.count()).select_from(User).where(User.role == "owner", User.is_active.is_(True))) or 0
    removing_last_owner = user.role == "owner" and user.is_active and owner_count <= 1 and (
...
def list_content(
    ...
    statement = select(ContentItem).options(selectinload(ContentItem.sources), selectinload(ContentItem.claims))
    if content_status is not None:
        ...
        statement = statement.where(ContentItem.status == content_status)
    items = list(db.scalars(statement.order_by(ContentItem.updated_at.desc()).limit(limit)).all())
    return [_content_summary(item) for item in items]
...
def add_source(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=409, detail="source key conflict") from exc
    item = _get_content(db, item.id)
    return ContentDetail.model_validate(item)
...
def add_claim(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=409, detail="claim key already exists") from exc
    item = _get_content(db, item.id)
    return ContentDetail.model_validate(item)
