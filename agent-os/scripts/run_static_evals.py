...
def main() -> int:
    ...
    for c in rows("injection.jsonl"):
        ...
        if got == c["suspicious"]: passed += 1
        else: failures.append(f"{c['id']}: injection {got} != {c['suspicious']}")
    ...
    for c in rows("completion.jsonl"):
        ...
        if dod.complete == c["complete"]: passed += 1
        else: failures.append(f"{c['id']}: completion {dod.complete} != {c['complete']}")
