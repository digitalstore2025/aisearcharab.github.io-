...
except ImportError as exc:raise SystemExit("Install dependencies with: pip install -r requirements.txt") from exc
from readiness.engine import Gate,Status,evaluate_invariants,export_csv,export_json,import_json_payload,load_snapshot,next_action_queue,parse_gates,production_decision,summarize
...
def dashboard()->None:
 ...
 def refresh_summary()->None:
  summary=summarize(state.gates,state.authoritative_release_validated);decision=production_decision(state.gates,state.authoritative_release_validated);refs["decision"].set_text(decision["decision"]);refs["reason"].set_text(decision["reason"]);refs["blocking"].set_text(pct(summary["blocking_gate_pass_rate"]));refs["verified"].set_text(pct(summary["verified_pass_rate"]));refs["trust"].set_text(pct(summary["trust_surface_completion"]));refs["blockers"].set_text(str(summary["blocking_not_pass"]))
 def refresh_table()->None:
  query=(refs["query"].value or "").strip().lower();status=refs["status"].value;gates=[g for g in state.gates if (status=="ALL" or g.status.value==status) and (not query or query in f"{g.id} {g.category} {g.gate} {g.evidence} {g.next_action}".lower())];refs["table"].rows=[table_row(g) for g in gates];refs["table"].update();refs["evidence_select"].options={g.id:f"{g.id} — {g.gate}" for g in state.gates};refs["evidence_select"].update()
  ...
  with container:
   for gate in next_action_queue(state.gates):
    with ui.row().classes("w-full items-start gap-2 border-b border-slate-100 py-2"):
     ...
     with ui.column().classes("gap-0"):ui.label(f"{gate.id} — {gate.gate}").classes("text-sm font-semibold");ui.label(gate.next_action or "Evidence/verification required.").classes("text-xs text-slate-500")
 def refresh_invariants()->None:
  container=refs["invariants"];container.clear()
  with container:
   for item in evaluate_invariants(state.gates):
    with ui.row().classes("w-full items-start justify-between gap-3 border-b border-slate-100 py-2"):ui.label(item["statement"]).classes("text-sm");ui.label(item["status"]).classes("text-xs font-bold")
 ...
 def show_evidence()->None:
  gate=next((item for item in state.gates if item.id==refs["evidence_select"].value),None)
 ...
 async def handle_upload(event)->None:
  try:
   ...
   state.gates=gates;state.authoritative_release_validated=False;state.release_evidence_info={};state.metadata={"source":"user-import","provenance":"original metadata intentionally cleared"};refresh_all();ui.notify(f"Imported {len(gates)} gate(s).",type="positive")
  except (UnicodeDecodeError,ValueError,json.JSONDecodeError) as exc:ui.notify(f"Import rejected: {exc}",type="negative")
 async def handle_release_evidence_upload(event)->None:
  try:
   raw=event.content.read(MAX_UPLOAD_BYTES+1)
   if len(raw)>MAX_UPLOAD_BYTES:ui.notify("Release evidence rejected: payload exceeds 1 MiB.",type="negative");return
   if isinstance(raw,bytes):raw=raw.decode("utf-8")
 ...
 with ui.column().classes("mx-auto w-full max-w-[1500px] gap-4 p-4 md:p-6"):
  with ui.row().classes("panel w-full items-center justify-between p-5"):
   with ui.column().classes("gap-0"):ui.label("aisearch.study").classes("text-xl font-extrabold");refs["subtitle"]=ui.label(TEXT["ar"]["subtitle"]).classes("text-sm text-slate-500");ui.label("تعلّم بذكاء").classes("text-xs font-semibold text-blue-600")
  ...
  with ui.card().classes("w-full border border-red-200 bg-red-50 shadow-none"):refs["decision"]=ui.label("NO-GO").classes("text-3xl font-black text-red-700");refs["reason"]=ui.label("").classes("text-sm");ui.label("GENERATED_ANSWERS_ENABLED=false").classes("font-mono text-xs")
  ...
  with ui.card().classes("panel w-full shadow-none"):
   with ui.row().classes("w-full items-center justify-between"):refs["live_status"]=ui.label("Live checks are opt-in and fail closed.").classes("text-xs text-slate-500");refs["live_button"]=ui.button(TEXT["ar"]["live"],on_click=live_scan).props("outline")
  with ui.card().classes("panel w-full shadow-none"):
   refs["gates_heading"]=ui.label(TEXT["ar"]["gates"]).classes("text-lg font-bold")
   with ui.row().classes("w-full gap-3"):refs["query"]=ui.input("Search").props("dense outlined clearable").classes("flex-1");refs["status"]=ui.select(["ALL","PASS","FAIL","PENDING","BLOCKED","UNKNOWN"],value="ALL",label="Status").props("dense outlined")
   refs["query"].on_value_change(lambda _:refresh_table());refs["status"].on_value_change(lambda _:refresh_table());columns=[{"name":"id","label":"ID","field":"id","align":"left"},{"name":"category","label":"Category","field":"category","align":"left"},{"name":"gate","label":"Gate","field":"gate","align":"left"},{"name":"status","label":"Status","field":"status","align":"left"},{"name":"blocking","label":"Blocking","field":"blocking","align":"left"},{"name":"verified","label":"Verified","field":"verified","align":"left"},{"name":"next_action","label":"Next action","field":"next_action","align":"left"}];refs["table"]=ui.table(columns=columns,rows=[],row_key="id",pagination=15).classes("w-full").props("flat bordered dense wrap-cells")
   with ui.row().classes("w-full items-end gap-2"):refs["evidence_select"]=ui.select({},label="Evidence record").classes("flex-1").props("outlined dense");ui.button("View evidence",on_click=show_evidence).props("outline")
  ...
  with ui.card().classes("panel w-full shadow-none"):
   ui.label("Authoritative release evidence must be validated separately from the gate matrix.").classes("text-xs font-semibold text-red-700")
   with ui.row().classes("items-center gap-2"):ui.upload(on_upload=handle_release_evidence_upload,auto_upload=True).props("accept=.json max-files=1 max-file-size=1048576");ui.label("release-evidence.py artifact").classes("text-xs text-red-700");ui.upload(on_upload=handle_upload,auto_upload=True).props("accept=.json max-files=1 max-file-size=1048576");refs["import_label"]=ui.label(TEXT["ar"]["import"]).classes("text-xs text-slate-500");json_button=ui.button(on_click=lambda:download_text("aisearch-readiness.json",export_json(state.gates,state.metadata,state.authoritative_release_validated))).props("outline")
