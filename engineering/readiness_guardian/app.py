...
class DashboardState:
    """Single-operator transient state; V1 is localhost-only by default."""
...
def row(gate: Gate) -> dict[str, str]:
...
def refresh_summary() -> None:
    ...
    refs["reason"].set_text(decision["reason"])
    refs["blocking"].set_text(pct(summary["blocking_gate_pass_rate"]))
    refs["verified"].set_text(pct(summary["verified_pass_rate"]))
    refs["trust"].set_text(pct(summary["trust_surface_completion"]))
    refs["blockers"].set_text(str(summary["blocking_not_pass"]))


def refresh_table() -> None:
    query = (refs["query"].value or "").strip().lower()
    ...
    refs["table"].rows = [row(gate) for gate in gates]
    refs["table"].update()
    refs["evidence_select"].options = {gate.id: f"{gate.id} — {gate.gate}" for gate in state.gates}
    refs["evidence_select"].update()


def refresh_actions() -> None:
    container = refs["actions"]
    container.clear()
    with container:
        for gate in next_action_queue(state.gates):
            with ui.row().classes("w-full items-start gap-2 border-b border-slate-100 py-2"):
                ...
                with ui.column().classes("gap-0"):
                    ui.label(f"{gate.id} — {gate.gate}").classes("text-sm font-semibold")
                    ui.label(gate.next_action or "Evidence/verification required.").classes("text-xs text-slate-500")


def refresh_invariants() -> None:
    container = refs["invariants"]
    container.clear()
    with container:
        for item in evaluate_invariants(state.gates):
            with ui.row().classes("w-full items-start justify-between gap-3 border-b border-slate-100 py-2"):
                ui.label(item["statement"]).classes("text-sm")
                ui.label(item["status"]).classes("text-xs font-bold")


def refresh_all() -> None:
    refresh_summary(); refresh_table(); refresh_actions(); refresh_invariants()


def set_locale(locale: str) -> None:
    state.locale = locale
    text = TEXT[locale]
    for key, ref_key in (("subtitle", "subtitle"), ("disclaimer", "disclaimer"), ("gates", "gates_heading"), ("actions", "actions_heading"), ("invariants", "invariants_heading"), ("live", "live_button"), ("import", "import_label"), ("json", "json_label"), ("csv", "csv_label"), ("restore", "restore_label")):
        refs[ref_key].set_text(text[key])
...
def show_evidence() -> None:
    ...
    if gate is None:
        ui.notify("Select a gate first.", type="warning"); return
    refs["dialog_title"].set_text(f"{gate.id} — {gate.gate}")
    refs["dialog_body"].set_text(f"Status: {gate.status.value}\n\nEvidence: {gate.evidence}\n\nAcceptance: {gate.acceptance or 'Not specified.'}\n\nNext action: {gate.next_action or 'None.'}")
    refs["dialog"].open()
...
async def handle_upload(event) -> None:
    try:
        ...
        if len(raw) > MAX_UPLOAD_BYTES:
            ui.notify("Import rejected: payload exceeds 1 MiB.", type="negative"); return
        if isinstance(raw, bytes): raw = raw.decode("utf-8")
        gates, errors = import_json_payload(raw)
        if errors or not gates:
            ui.notify("Import rejected: " + "; ".join(errors[:3]), type="negative"); return
        state.gates = gates
        state.metadata = {"source": "user-import", "provenance": "original metadata intentionally cleared"}
        refresh_all()
        ui.notify(f"Imported {len(gates)} gate(s).", type="positive")
    except (UnicodeDecodeError, ValueError, json.JSONDecodeError) as exc:
        ui.notify(f"Import rejected: {exc}", type="negative")
...
def restore_seed() -> None:
    state.restore(); refresh_all(); ui.notify("Seed snapshot restored.", type="positive")
...
async def live_scan() -> None:
    ...
    try:
        ...
        retained = [gate for gate in state.gates if not gate.id.startswith(("LIVE-", "TRUST-"))]
        state.gates = retained + live
        refresh_all(); refs["live_status"].set_text(f"Live scan complete: {len(live)} records.")
    except Exception as exc:
        refs["live_status"].set_text(f"Live scan failed safely: {exc}")
    finally:
        refs["live_button"].enable()
...
with ui.column().classes("mx-auto w-full max-w-[1500px] gap-4 p-4 md:p-6"):
    with ui.row().classes("panel w-full items-center justify-between p-5"):
        with ui.column().classes("gap-0"):
            ui.label("aisearch.study").classes("text-xl font-extrabold")
    ...
    with ui.grid(columns=4).classes("w-full gap-3 max-md:grid-cols-2 max-sm:grid-cols-1"):
        for title, key in (("Blocking Gate Pass Rate", "blocking"), ("Verified Pass Rate", "verified"), ("Trust Surface Completion", "trust"), ("Blocking Gates Not Pass", "blockers")):
            with ui.card().classes("metric shadow-none"):
                ui.label(title).classes("text-xs text-slate-500"); refs[key] = ui.label("0").classes("text-3xl font-extrabold")
    ...
    with ui.grid(columns=2).classes("w-full gap-3 max-lg:grid-cols-1"):
        with ui.card().classes("panel shadow-none"):
            refs["actions_heading"] = ui.label(TEXT["ar"]["actions"]).classes("text-lg font-bold"); refs["actions"] = ui.column().classes("w-full gap-0")
        with ui.card().classes("panel shadow-none"):
            refs["invariants_heading"] = ui.label(TEXT["ar"]["invariants"]).classes("text-lg font-bold"); refs["invariants"] = ui.column().classes("w-full gap-0")
    with ui.card().classes("panel w-full shadow-none"):
