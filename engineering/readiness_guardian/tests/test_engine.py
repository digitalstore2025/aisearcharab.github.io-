...
def test_arbitrary_verified_passes_cannot_bypass_mandatory_registry():
 assert production_decision([gate("A",Status.PASS,verified=True),gate("B",Status.PASS,verified=True)])["decision"]=="NO-GO"

def test_full_mandatory_registry_verified_pass_allows_go():
 ...
 assert production_decision(gates)["decision"]=="GO"
...
def test_summary_metrics_are_computed_not_hardcoded():
 gates=[gate("A",Status.PASS,verified=True),gate("B",Status.PENDING),gate("C",Status.PASS,blocking=False,category="Search/Trust")];summary=summarize(gates)
 assert summary["blocking_total"]==2 and summary["blocking_pass"]==1 and summary["blocking_gate_pass_rate"]==0.5 and summary["verified_pass_rate"]==1/3
