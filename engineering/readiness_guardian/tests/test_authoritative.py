...
def _fake_repo(tmp_path):
    repo = tmp_path
    script = repo / "platform" / "apps" / "api" / "scripts" / "release_evidence.py"
...
def test_authoritative_artifact_must_match_expected_sha_and_ref(tmp_path):
    ...
    payload = json.dumps({"status": "PRODUCTION_READY", "source_ref": "refs/heads/main", "source_head_sha": sha, "tested_sha": sha, "blockers": []})
    with pytest.raises(ValueError, match="Explicit expected SHA"):
...
def test_authoritative_artifact_must_match_expected_ref(tmp_path):
    repo, sha = _fake_repo(tmp_path)
    payload = json.dumps({"status": "PRODUCTION_READY", "source_ref": "refs/heads/release-old", "source_head_sha": sha, "tested_sha": sha, "blockers": []})
    with pytest.raises(ValueError):
        validate_release_evidence_artifact(payload, repo, expected_sha=sha, expected_source_ref="refs/heads/release-old")
...
def test_explicit_expected_sha_is_never_silently_overwritten(tmp_path):
    repo = tmp_path / "repo-explicit"; repo.mkdir()
    subprocess.run(["git", "init", "-b", "main"], cwd=repo, check=True, capture_output=True)
    subprocess.run(["git", "config", "user.email", "test@example.com"], cwd=repo, check=True)
    subprocess.run(["git", "config", "user.name", "Test"], cwd=repo, check=True)
    (repo / "x.txt").write_text("x", encoding="utf-8")
    ...
    subprocess.run(["git", "commit", "-m", "init"], cwd=repo, check=True, capture_output=True)
    with pytest.raises(ValueError, match="Explicit expected SHA"):
        validate_release_evidence_artifact("{}", repo, expected_sha="f" * 40, expected_source_ref=None)
