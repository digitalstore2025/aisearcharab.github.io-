...
def run_live_checks(base_url: str) -> list[Gate]:
    ...
    try:
        ...
        http_ok = 200 <= status < 400 and final.scheme == "https" and final.hostname == hostname
        ...
        parser = _CanonicalParser(); parser.feed(body.decode("utf-8", errors="replace")); expected = base_url.rstrip("/")
    ...
    for path, blocking in trust_paths.items():
        ...
        try:
            ...
            final = urllib.parse.urlparse(final_url)
            ok = 200 <= status < 300 and bool(body.strip()) and final.scheme == "https" and final.hostname == hostname
            evidence = f"HTTP {status}; bytes={len(body)}; final_url={final_url}"
        except Exception as exc:
            ok = False; evidence = f"Check failed: {exc}"
        results.append(Gate(id="TRUST-" + path.strip("/").replace(".", "-").upper(), category="Search/Trust", gate=f"Trust surface {path}", status=Status.PASS if ok else Status.FAIL, blocking=blocking, verified=ok, trust_surface=True, evidence=evidence, source=url, acceptance=f"{path} exists on the expected HTTPS origin and returns substantive content.", next_action="" if ok else f"Publish or repair {path}."))
    return results
