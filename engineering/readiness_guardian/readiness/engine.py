...
MANDATORY_BLOCKING_GATE_IDS = frozenset({"SEC-REG", "SEC-DIFF", "SEARCH-REG", "OPENAPI", "RATE-LIMIT", "OBSERVABILITY", "HC-FINDINGS", "SECRET-MGR", "PROD-ACT"})
SECURITY_INVARIANTS = {
...
def _normalize_gate(raw: dict[str, Any]) -> Gate:
    ...
    return Gate(
        id=raw["id"].strip(), category=raw["category"].strip(), gate=raw["gate"].strip(), status=status,
        blocking=raw["blocking"], evidence=raw["evidence"].strip(), source=str(raw.get("source", "")).strip(),
...
def parse_gates(snapshot: dict[str, Any]) -> list[Gate]:
    seen: set[str] = set(); gates: list[Gate] = []
...
def import_json_payload(text: str) -> tuple[list[Gate], list[str]]:
    ...
    accepted: list[Gate] = []; errors: list[str] = []; seen: set[str] = set()
...
def summarize(gates: Iterable[Gate]) -> dict[str, Any]:
    gates = list(gates); blocking = [g for g in gates if g.blocking]; blocking_pass = [g for g in blocking if is_pass(g)]
    verified_pass = [g for g in gates if is_verified_pass(g)]; trust = [g for g in gates if g.trust_surface or g.category == "Search/Trust"]; trust_pass = [g for g in trust if is_pass(g)]
    decision = production_decision(gates)
    def ratio(n: int, d: int) -> float: return 0.0 if d == 0 else n / d
    return {"blocking_gate_pass_rate": ratio(len(blocking_pass), len(blocking)), "verified_pass_rate": ratio(len(verified_pass), len(gates)), "trust_surface_completion": ratio(len(trust_pass), len(trust)), "blocking_total": len(blocking), "blocking_pass": len(blocking_pass), "blocking_not_pass": len(blocking) - len(blocking_pass), "production_go": decision["decision"] == "GO", "decision": decision["decision"], "decision_reason": decision["reason"]}
...
def next_action_queue(gates: Iterable[Gate]) -> list[Gate]:
    def key(g: Gate) -> tuple[int, int, str]: return (0 if g.blocking else 1, _STATUS_WEIGHT[g.status], g.id)
    return sorted((g for g in gates if not is_pass(g)), key=key)


def evaluate_invariants(gates: Iterable[Gate]) -> list[dict[str, Any]]:
    by_id = {g.id: g for g in gates}; results: list[dict[str, Any]] = []
    for invariant_id, cfg in SECURITY_INVARIANTS.items():
        ...
        blockers = [gate for gate in dependencies if gate is not None and not is_verified_pass(gate)]
